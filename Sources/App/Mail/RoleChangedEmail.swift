...
enum RoleChangedEmail {
    ...
    private static func copy(for lang: String?, name: String, role: UserRole) -> Copy {
        ...
        case "it": return Copy(subject: "Il tuo ruolo su FontApp è cambiato", greeting: "Ciao, \(name)", body: "Il team di FontApp ti ha assegnato un nuovo ruolo.", roleLabel: "Nuovo ruolo: \(roleName(role, lang: "it"))", permissionsTitle: "Cosa puoi fare adesso?", permissions: permissions, note: "Usa questi permessi con attenzione: le azioni di moderazione vengono registrate per proteggere la comunità.", cta: "Apri FontApp", footer: "Ricevi questa email perché il proprietario di FontApp ha cambiato il ruolo del tuo account.")
    ...
    private static func permissions(for role: UserRole, lang: String) -> [String] {
        ...
        case "it": copy = ("Usare le funzioni normali di FontApp e gestire i tuoi contenuti.", ["Esaminare la coda di moderazione e le segnalazioni della comunità.", "Nascondere fontane false, offensive o spam e ripristinarle se è stato un errore.", "Moderare recensioni, segnalazioni e foto pubblicate da altre persone."], ["Include tutti i permessi di moderatore.", "Gestire le fontane: correggere posizioni, foto e cambiamenti, e annullare modifiche.", "Risolvere richieste amministrative e consultare statistiche e cronologia interna."], "Puoi anche assegnare ruoli e gestire le impostazioni globali di FontApp.")
    ...
    private static func roleName(_ role: UserRole, lang: String) -> String {
        ...
        case ("it", .user): return "Utente"
        case ("it", .moderator): return "Moderatore"
        case ("it", .admin): return "Amministratore"
