...
final class IntegrationTests: XCTestCase {
    ...
    func testCommentIsNotAnIncidentUntilItIsMarked() async throws {
        try await withApp { app in
            ...
            try await app.test(.POST, "fonts/\(fontID)/report", headers: bearer(token), beforeRequest: { req in
                try req.content.encode(CreateReportDTO(message: "¿le pones una foto?", isIncident: nil, incidentKind: nil, parentID: nil, duplicateOf: nil))
            }, afterResponse: { res in
                XCTAssertEqual(res.status, .created)
    ...
    func testReportRecordsAuthor() async throws {
        try await withApp { app in
            ...
            try await app.test(.POST, "fonts/\(fontID)/report", headers: bearer(token), beforeRequest: { req in
                try req.content.encode(CreateReportDTO(message: "grifo roto", isIncident: true, incidentKind: .broken, parentID: nil, duplicateOf: nil))
            }, afterResponse: { res in
                XCTAssertEqual(res.status, .created)
    ...
    func testResolvingAnIncidentKeepsItAndCanBeUndone() async throws {
        try await withApp { app in
            ...
            try await app.test(.POST, "fonts/\(fontID)/report", headers: bearer(tok), beforeRequest: { req in
                try req.content.encode(CreateReportDTO(message: "El caño está roto", isIncident: true, incidentKind: .broken, parentID: nil, duplicateOf: nil))
            }, afterResponse: { res in
                XCTAssertEqual(res.status, .created)
    ...
    func testReportingAFountainFollowsIt() async throws {
        try await withApp { app in
            ...
            try await app.test(.POST, "fonts/\(fontID)/report", headers: bearer(tok), beforeRequest: { req in
                try req.content.encode(CreateReportDTO(message: "El agua no es potable", isIncident: true, incidentKind: .other, parentID: nil, duplicateOf: nil))
            }, afterResponse: { res in XCTAssertEqual(res.status, .created) })
            ...
            try await app.test(.POST, "fonts/\(fontID)/report", headers: bearer(tok), beforeRequest: { req in
                try req.content.encode(CreateReportDTO(message: "Y además está rota", isIncident: true, incidentKind: .broken, parentID: nil, duplicateOf: nil))
            }, afterResponse: { res in XCTAssertEqual(res.status, .created) })
...
/// «Demarcaciones» premia haber recorrido territorio, así que tampoco se gana desde el
    ...
    func testAFlowingReviewClosesOpenIncidentsByItself() async throws {
        try await withApp { app in
            ...
            try await app.test(.POST, "fonts/\(fontID)/report", headers: bearer(token), beforeRequest: { req in
                try req.content.encode(CreateReportDTO(message: "Está seca desde julio", isIncident: true, incidentKind: .dry, parentID: nil, duplicateOf: nil))
            }, afterResponse: { res in XCTAssertEqual(res.status, .created) })
